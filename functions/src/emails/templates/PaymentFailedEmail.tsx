...
interface PaymentFailedProps { name: string; }

export const PaymentFailedEmail = ({ name }: PaymentFailedProps) => {
  return (
    <Html>
        ...
        <Body className="bg-surface text-onSurface font-body m-0 p-0 antialiased py-8">
          <Container className="max-w-[600px] mx-auto p-4 md:p-6">
            ...
            <main className="px-2">
              ...
              <Section className="bg-white p-10 rounded-[24px] mb-8 border border-red-100 shadow-sm relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-red-500/10 rounded-full blur-2xl -mr-10 -mt-10"></div>
                <Text className="text-2xl font-bold font-headline mb-4 text-[#0f172a] relative z-10">Payment Update</Text>
                <Text className="text-[16px] font-body text-onSurfaceVariant leading-relaxed mb-6 relative z-10">
                  Hi {name}, we attempted to process your subscription renewal, but the payment failed. Your subscription is now marked as <strong className="text-error">past due</strong>.
                </Text>
                <Text className="text-[16px] font-body text-onSurfaceVariant leading-relaxed mb-8 relative z-10">
                  To maintain uninterrupted access to your AI Tutor and premium courses, please update your payment method or ensure sufficient funds are available.
                </Text>
                <Button className="inline-flex items-center justify-center px-8 py-3 bg-[#e11d48] text-white font-bold font-body rounded-xl shadow-sm relative z-10" href="https://mytutorme.org/settings">
