...
export const CourseApprovalEmail = ({ teacherName, courseTitle, courseUrl }: CourseApprovalEmailProps) => {
  return (
    <Html>
      <Preview>Your Course is Live!</Preview>
      <Tailwind config={{
          theme: {
            extend: {
              colors: { 
        ...
        <Body className="bg-surface text-onSurface font-body m-0 p-0 antialiased py-8">
          <Container className="max-w-[600px] mx-auto p-4 md:p-6">
            ...
            <main className="px-2">
              ...
              <Section className="text-center bg-primary/10 p-10 rounded-[24px] mb-8 border border-primary/20">
                <Text className="text-3xl font-extrabold font-headline mb-4 text-primary">Your Course is LIVE!</Text>
                <Text className="text-[16px] font-body text-onSurfaceVariant leading-relaxed mb-6">
                  Fantastic news, {teacherName}. <strong>"{courseTitle}"</strong> has passed our QA review and is officially available globally!
                </Text>
                <Button className="inline-flex px-8 py-4 bg-primary text-white font-bold rounded-xl shadow-md" href={courseUrl}>
                  View Course Details
                </Button>
              </Section>
            </main>
