...
export const CourseDetailsPage = () => {
    ...
    const fetchCourseData = async () => {
      ...
      try {
        ...
        if (courseSnap.exists()) {
          ...
          // Dummy fallback if not in DB
          setCourse({
            id: courseId,
            title: "Advanced Calculus & Linear Algebra",
            description: "Master derivatives, integrals, and vector spaces with AI assistance.",
            teacherName: "Dr. Funke Adebayo",
            price: 49000,
            rating: 4.8,
        ...
        // 2. Fetch Modules (from subcollection, minimal mock if none)
        const modulesRef = collection(db, \`courses/\${courseId}/modules\`);
        const modulesQ = query(modulesRef);
        const modulesSnap = await getDocs(modulesQ);
        if (!modulesSnap.empty) {
            const modsData = modulesSnap.docs.map(mod => ({ id: mod.id, ...mod.data() } as unknown as Module));
            setModules(modsData.sort((a,b)=>a.order - b.order));
        } else {
            // Dummy subcollection fallback
            setModules([
                {
                    id: "mod-1",
                    title: "Module 1: Foundations of Limits",
                    order: 1,
                    lessons: [
                        { id: "les-1", title: "Introduction to Limits", durationMinutes: 10 },
                        { id: "les-2", title: "Evaluating Limits Algebraically", durationMinutes: 15 }
                    ]
                },
                {
                    id: "mod-2",
                    ...
                    order: 2,
                    lessons: [
                        { id: "les-3", title: "Power Rule & Chain Rule", durationMinutes: 22 },
                        { id: "les-4", title: "Derivatives of Trig Functions", durationMinutes: 18 }
                    ]
                }
            ]);
        }
  ...
  if (activeLesson) {
    return <LessonPlayer lessonTitle={activeLesson.title} onExit={() => setActiveLesson(null)} onComplete={handleLessonComplete} />;
  }
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm text-slate-500 dark:text-slate-400">
              <div className="flex items-center gap-1">
                ...
                <span className="font-bold text-slate-900 dark:text-white ml-1">{course.rating || 4.5}</span>
              </div>
                ...
                <div className="p-6">
                   {activeTab === 'curriculum' && (
                      <div className="space-y-6">
                         {modules.map((mod) => {
                             const isModCompleted = mod.lessons && mod.lessons.every(l => completedLessons.includes(l.id));
                             return (
                                ...
                                <div className="divide-y divide-gray-100 dark:divide-gray-700">
                                    {mod.lessons && mod.lessons.map(lesson => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {isCompleted ? <CheckCircle className="w-5 h-5 text-green-500" /> : <PlayCircle className="w-5 h-5 text-primary" />}
                                                <span className={\`\${isCompleted ? 'text-gray-600 dark:text-gray-400' : 'text-gray-900 dark:text-white font-medium'} text-sm\`}>{lesson.title}</span>
                                                <span className="ml-auto text-xs text-gray-500">{lesson.durationMinutes}:00</span>
                                            </div>
                                        )
                                    })}
                                </div>
                      ...
                      <div className="space-y-4">
                         <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-5 flex items-center justify-between hover:border-primary transition-colors bg-white dark:bg-slate-800 shadow-sm">
                            <div className="flex gap-4 items-center">
                               <div className="h-12 w-12 rounded-full bg-primary/10 text-primary flex items-center justify-center"><Play className="w-6 h-6 ml-1" /></div>
                               <div><h3 className="font-semibold text-gray-900 dark:text-white">Derivatives Checkpoint</h3><p className="text-sm text-gray-500">Practice your knowledge of basic rules.</p></div>
                            </div>
                            <Button onClick={() => setActiveQuiz(MOCK_QUIZ)}>Start Quiz</Button>
                         </div>
                      </div>
                   )}
...
fs.writeFileSync('./src/features/student/pages/CourseDetailsPage.tsx', content, 'utf8');
console.log("Successfully wrote CourseDetailsPage.tsx!");